...
import { FiPlus, FiEdit2, FiTrash2, FiX, FiUpload, FiFileText, FiAlertCircle } from "react-icons/fi";
...
export default function AlasanLostPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/master/alasan-lost?${params.toString()}`);
  ...
  const handleOpenModal = (alasan = null) => {
    if (alasan) {
      setFormData({ id: alasan.id, kode: alasan.kode, nama: alasan.nama, aktif: alasan.aktif });
    } else {
  ...
  const handleDelete = (id) => {
    showConfirm("Konfirmasi Hapus", "Apakah Anda yakin ingin menghapus data alasan lost ini?",
      async () => {
        try {
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      { KODE: "AL01", NAMA_ALASAN: "Harga Terlalu Mahal", STATUS: "Aktif" },
      { KODE: "AL02", NAMA_ALASAN: "Stok Kosong", STATUS: "Aktif" }
    ];
  ...
  const columns = useMemo(() => [
    {
      ...
      width: 60,
      render: (row) => {
        const idx = alasanLosts.findIndex(a => a.id === row.id);
  ...
  ], [alasanLosts, tableState.page, tableState.pageSize]);
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        emptyText="Belum ada Alasan Lost."
        rowKey="id"
        ...
        searchPlaceholder="Cari kode atau nama alasan..."
