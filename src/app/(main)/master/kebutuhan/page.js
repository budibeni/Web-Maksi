...
export default function KebutuhanPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/master/kebutuhan?${params.toString()}`);
      const json = await res.json();
      if (json.success) {
        setKebutuhans(json.data || []);
        if (json.pagination) applyPagination(json.pagination);
      }
    } catch (error) {
      console.error(error);
      showToast("Gagal mengambil data kebutuhan", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = formData.id ? `/api/master/kebutuhan/${formData.id}` : "/api/master/kebutuhan";
  ...
  const handleDelete = (id) => {
    showConfirm("Konfirmasi Hapus", "Apakah Anda yakin ingin menghapus data kebutuhan ini?",
      async () => {
        try {
          const res = await fetch(`/api/master/kebutuhan/${id}`, { method: "DELETE" });
          const json = await res.json();
          if (res.ok && json.success) {
            fetchData();
            showToast(json.message, "success");
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      { KODE: "KEB01", NAMA_KEBUTUHAN: "Hanya tanya-tanya", FASE_LEAD: "LEAD_BARU", URUTAN: 1, WARNA: "blue", STATUS: "Aktif" },
      { KODE: "KEB02", NAMA_KEBUTUHAN: "Menunggu keputusan", FASE_LEAD: "FOLLOW_UP", URUTAN: 2, WARNA: "yellow", STATUS: "Aktif" },
      { KODE: "KEB03", NAMA_KEBUTUHAN: "Minta penawaran", FASE_LEAD: "PENAWARAN", URUTAN: 3, WARNA: "purple", STATUS: "Aktif" }
    ];
    exportToExcel(templateData, "template_kebutuhan.xlsx");
  };
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        const row = data[i];
        if (!row.KODE || !row.NAMA_KEBUTUHAN || !row.FASE_LEAD) continue;
        ...
        const payload = {
          kode: row.KODE.toString().trim(), nama: row.NAMA_KEBUTUHAN.toString().trim(),
        ...
        const res = await fetch("/api/master/kebutuhan", {
  ...
  const columns = useMemo(() => [
    {
      ...
      width: 60,
      render: (row) => {
        const idx = kebutuhans.findIndex(k => k.id === row.id);
  ...
  ], [kebutuhans, tableState.page, tableState.pageSize]);
        ...
        headerActions={
          <>
            ...
            >
              ...
              <span className="hidden sm:inline">Tambah Kebutuhan</span>
            </button>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95">
            <div className="px-6 py-4 border-b border-neutral-100 dark:border-neutral-800 flex justify-between items-center">
              <h3 className="text-lg font-bold text-neutral-900 dark:text-white">
                {formData.id ? "Edit Kebutuhan" : "Tambah Kebutuhan"}
              </h3>
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Nama Kebutuhan *</label>
