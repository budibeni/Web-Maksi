...
export default function ProdukPage() {
  ...
  const handleOpenModal = (produk = null) => {
    if (produk) {
      ...
      const defaultKategori = kategoriList.length > 0 ? kategoriList[0].id : "";
      setFormData({ 
        id: null, 
  ...
  const kategoriFilterOptions = useMemo(() => {
    return kategoriList.map(k => ({ value: String(k.id), label: k.nama }));
  }, [kategoriList]);
  ...
  const columns = useMemo(() => [
    {
      ...
      width: 80,
      render: (row) => {
        const idx = produks.findIndex(p => p.id === row.id);
    ...
    {
      key: "kategori_produk_id",
      label: "Kategori",
      sortable: true,
      filter: { type: "select", options: kategoriFilterOptions },
      render: (row) => (
    ...
    },
    {
      ...
      filter: { type: "text" },
      render: (row) => (
        <span className="text-neutral-900 dark:text-neutral-200 min-w-[200px] inline-block font-semibold">
          {row.nama}
        </span>
      )
    },
    ...
    },
    {
      ...
      render: (row) => (
        <div className="text-right font-medium text-neutral-900 dark:text-white">
          Rp {Number(row.harga_default).toLocaleString('id-ID')}
        </div>
      )
    },
    {
  ...
  ], [produks, tableState.page, tableState.pageSize, kategoriFilterOptions]);
  ...
  return (
    <div className="space-y-6">
      <DataTable
        ...
        data={produks}
        isLoading={isLoading}
        emptyText="Tidak ada data produk ditemukan."
        rowKey="id"
        // Pagination
